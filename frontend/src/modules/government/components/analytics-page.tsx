...
export function AnalyticsPage() {
  ...
  const handleBulkRelease = () => {
    ...
    farmers.forEach((f) => {
      ...
      const bonus =
        f.farmingMethod === "organic" ? creditReleaseFormula.organicBonus
        : f.farmingMethod === "transitioning" ? creditReleaseFormula.transitioningBonus
        : 0;
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6">
      <PageHeader
        title="Biosphere Health"
        subtitle="Monitoring regional vital signs and funding distribution across 12 bio-zones."
      />

      {/* Top stats */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <StatCard label="Species Logged" value="12K+" icon={Bug} color="text-[var(--green-deep)]" />
        <StatCard label="Organic Farming" value={`${organicPct}%`} icon={Leaf} color="text-[var(--green-light)]" />
        <StatCard label="Total Credits" value={totalCredits} icon={Star} color="text-[var(--amber-mid)]" />
        <StatCard label="Active Nodes" value="82" icon={TrendingUp} color="text-blue-600" />
      </div>

      <div className="grid grid-cols-1 xl:grid-cols-2 gap-5">
        {/* Biosphere zone health */}
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            {BIOSPHERE_ZONES.map((zone) => (
              <div key={zone.id}>
                ...
                <ProgressBar value={zone.health} color={STATUS_COLOR[zone.status].bar} showLabel />
              </div>
            ))}

            <AlertStrip message="Zone A-12 is at critical health. Recommend immediate intervention and irrigation funding allocation." />
          </CardContent>
        </Card>

        {/* Credit release formula */}
        <Card>
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            ...
            {[
              { label: "Credits per Capture", key: "perCapture" as const, value: creditReleaseFormula.perCapture, min: 1, max: 50 },
              { label: "Organic Farming Bonus", key: "organicBonus" as const, value: creditReleaseFormula.organicBonus, min: 0, max: 1000 },
              { label: "Transitioning Bonus", key: "transitioningBonus" as const, value: creditReleaseFormula.transitioningBonus, min: 0, max: 500 },
              ...
              <div key={key}>
                ...
                <input
                  ...
                  max={max}
                  value={value}
                  onChange={(e) => updateCreditFormula({ [key]: parseInt(e.target.value) })}
            ...
            <Button onClick={handleBulkRelease} disabled={releasing} className="w-full" size="lg">
              {releasing ? (
              ...
              >
                ...
                <div className="text-xs text-[var(--text-muted)] space-y-1">
                  {[
                    ...
                    { label: "Credits", value: farmer.carbonCredits.toLocaleString(), highlight: true },
                  ].map(({ label, value, highlight }) => (
                    <div key={label} className="flex justify-between">
                      <span>{label}</span>
                      <span className={highlight ? "font-semibold text-[var(--green-deep)]" : "font-medium text-[var(--text-dark)]"}>
                        {value}
                      </span>
                    </div>
                  ))}
      ...
      <Toast
        show={released}
        message={`Credits released to all ${farmers.length} farmers!`}
        icon={Send}
      />
    </div>
  );
