...
CORE_REQUIRED_VERSION=("3.29.1" "3.30.0" "3.31.0")
...
if is_sourced; then
    ...
    # Add Hailo RPi common modules to PYTHONPATH
    # Try standard TAPPAS locations
    for HAILO_PATH in /usr/share/hailo-tappas-core/apps/h8/python /usr/local/share/hailo-tappas-core/apps/h8/python /opt/hailo/apps /opt/tappas/apps/h8/gstreamer/libs; do
        if [ -d "$HAILO_PATH" ]; then
            export PYTHONPATH="${HAILO_PATH}:${PYTHONPATH:-}"
            echo "Added Hailo modules from $HAILO_PATH to PYTHONPATH"
            break
        fi
    done
